...
$$
\min_{\phi} \mathbb{E}_{\left(\mathbf{s}, \mathbf{a}, \mathbf{s}'\right) \sim \mathcal{D}}\left[\left(Q_{\phi}(\mathbf{s}, \mathbf{a})-\left(r(\mathbf{s}, \mathbf{a})+\gamma \mathbb{E}_{\mathbf{a}' \sim \pi_{\theta}\left(\cdot \mid \mathbf{s}'\right)}\left[Q_{\phi'}\left(\mathbf{s}', \mathbf{a}'\right)\right]  - \beta \log \pi_{\theta}\left(\mathbf{a}' \mid \mathbf{s}'\right) \right)\right)^{2}\right]
$$
...
$$
\text{Var}(Q_{\phi_j}(s, a + kw_2)) \approx \text{Var}(Q_{\phi_j}(s, a) + k \langle w_2, \nabla_a Q_{\phi_j}(s, a) \rangle)
$$

$$
= \text{Var}(Q(s, a) + k \langle w_2, \nabla_a Q_{\phi_j}(s, a) \rangle)
$$

$$
= k^2 \text{Var}(\langle w_2, \nabla_a Q_{\phi_j}(s, a) \rangle)
$$
...
Where $w_{\text{min}}$ and $\lambda_{\text{min}}$ are the smallest eigenvector and eigenvalue for the matrix $\text{Var}(\nabla_a Q_{\phi_j}(s, a))$
...
Let $q_j = \nabla_a Q_{\phi_j}(s, a)$ be the normalized gradients of Q-network, and $\bar{q} = \frac{1}{N} \sum_{j}$

$$
= \frac{1}{|\mathcal{A}|} \text{tr}(\frac{1}{N} \sum_{j} (q_j - \bar{q}) (q_j - \bar{q})^{\top})
$$

$$
= \frac{1}{|\mathcal{A}|} \frac{1}{N} \sum_{j} \text{tr}((q_j - \bar{q}) (q_j - \bar{q})^{\top})
$$

---

$$
= \frac{1}{|\mathcal{A}|} \frac{1}{N} \sum_{j} \text{tr}((q_j - \bar{q})^{\top} (q_j - \bar{q}))
$$
...
$$
= \frac{1}{|\mathcal{A}| N} \sum_{j} (q_j^{\top} q_j + \bar{q}^{\top} \bar{q} - 2 q_j^{\top} \bar{q})
$$

$$
= \frac{1}{|\mathcal{A}|} (1 + \bar{q}^{\top} \bar{q} - 2 ( \frac{1}{N} \sum_{j} q_j^{\top}) \bar{q})
$$
...
$$
= k^2 w_{\text{min}}^{\top} \text{Var}(\nabla_a Q_{\phi_j}(s, a)) w_{\text{min}}
$$

$$
= k^2 \lambda_{\text{min}} 
\leq \frac{1}{|\mathcal{A}|} (1 - \langle \frac{1}{N} \sum_{i=1}^{N} q_i, \frac{1}{N} \sum_{j=1}^{N} q_j \rangle)
$$
