...
We aim to learn a policy $\pi$ from the history of trajectories $\mathcal{D} = \{ s_t, a_t, s_t', r_t \}$ generated by behavioral policy $\pi_{\beta}$ s.t. the performance $\pi \geq \pi_{\beta}$
...
- Policy Constraints: Limit the distance of the policy distribution between agent's policy $\pi(a | s)$ and behavioral policy $\pi_{\beta}(a | s)$(the policy that generate the training dataset)
...
$s_t$ is state, $a_t$ is action and, $r_t$ is reward at time step t.
